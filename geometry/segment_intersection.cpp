...
bool isInRange(pair<double, double> X, pair< pair<double, double>, pair<double, double> > R) {
...
bool isInsideSegment(pair<double, double> point, pair< pair<double, double>, pair<double, double> > segment) {
  if(isEqual(getDistance(point, segment.first) + getDistance(point, segment.second),  getDistance(segment.first, segment.second)))
    return true;
  else
    return false;
}
...
int main() {
    ...
    if(determinant != 0) {
        double x = (double)-1 * (double)getDeterminant({ecA.c, ecA.b}, {ecB.c, ecB.b});
        x /= (double)determinant;
        double y = (double)-1 * (double)getDeterminant({ecA.a, ecA.c}, {ecB.a, ecB.c});
