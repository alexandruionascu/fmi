...
lungimePlus xs n = (replicate n 0) ++ xs
...
normalizeazaLungime (xs, ys) = let maxLen = max (length xs) (length ys) in
     (lungimePlus xs (maxLen - (length xs)), lungimePlus ys (maxLen - (length ys)))
...
lte xs ys = let (a, b) = normalizeazaLungime (xs, ys) in 
            a `lteN` b
...
propagaFold' xs = foldl (\acc x -> (x  `div` 10, [(x + fst acc) `mod` 10] ++ (snd acc))) (0, []) $ reverse xs
...
propagaFold xs = let xas = propagaFold' xs in (fst xas) : (snd xas)
...
plus xs ys = let (a, b) = normalizeazaLungime (xs, ys) in 
        propagaFold $ plusLista a b
...
minusLista xs ys = [fst x - snd x | x <- zip xs ys]
...
minus xs ys = let (a, b) = normalizeazaLungime (xs, ys)
                  minusList = propagaFold $ a `minusLista` b in
                  if (length $ filter (< 0) minusList) == 0 then minusList else error "Numere negative neimplementate" 
...
mulC x xs = propagaFold $ map (x*) xs
...
mul xs ys = snd $ foldr (\x acc -> let (p, current) = acc in (p + 1, current `plus` ((mulC x ys) ++ replicate p 0) )) (0, []) xs
